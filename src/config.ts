...
export const config = {
  ...
  /**
   * @deprecated Legacy knob. Read but never consumed — no code path loads a
   * wallet, builds a transaction, or signs. This build is dry-run only;
   * setting it changes nothing (see dryRun below).
   */
  privateKey: env("PRIVATE_KEY"),
  /**
   * Always true in this build. `Market.init()` refuses to run when false, so
   * dry run is a hard guarantee, not a toggle: if a private key is set
   * without DRY_RUN=true, startup throws instead of going live.
   */
  ...
  /**
   * Legacy knob (microlamports). No live transactions exist in this build —
   * the dry-run simulator always reports feeSol: 0. Kept for config-shape
   * compatibility only.
   */
  priorityFeeMicroLamports: Number(env("PRIORITY_FEE_MICROLAMPORTS", "10000")),
  /** Give up on a tx with no confirmation after this many slots. (No txs are constructed in this build.) */
