...
import { fmtConf, fmtSol, fmtPrice, fmtSigned, fmtSignedSol } from "@/lib/format";
import { smoothPath } from "./smooth";
import styles from "./FlowChart.module.css";

const STEP = 10; // px per slot
const ANCHOR_GAP = 88; // newest point sits this far from the right edge
const PAD_TOP = 84; // overlays live here
const PAD_BOTTOM = 72; // slot strip + tag clearance
const EASE = 0.1; // scale easing per new slot
...
export default function FlowChart({
  ...
  const scaleRef = useRef<{ lo: number; hi: number; slot: number } | null>(null);
  ...
  const model = useMemo(() => {
    ...
    if (latest && (!tail || latest.slot > tail.slot)) series = [...series, latest].slice(-n);
    else if (latest && tail && latest.slot === tail.slot) series[series.length - 1] = latest;
    const last = series[series.length - 1];
    if (!last) return null;

    if (originRef.current === null) originRef.current = series[0].slot;
    const origin = originRef.current;
    const fx = (b: number) => (b - origin) * STEP;

    // --- value scale: window min/max, floored to 0.20% of price, eased 10%/slot
    let lo = Infinity;
    let hi = -Infinity;
    let sum = 0;
    ...
    scaleRef.current = { lo, hi, slot: last.slot };

    const range = hi - lo || 1;
    const fy = (p: number) => PAD_TOP + (1 - (p - lo) / range) * plotH;

    const pts = series.map((e) => [fx(e.slot), fy(e.mid)] as const);
    ...
    const bySlot = new Map(series.map((e) => [e.slot, e]));
    ...
    return {
      ...
      last,
      base,
      shift: w - ANCHOR_GAP - fx(last.slot),
      endY: fy(last.mid),
    };
  }, [events, latest, w, h]);
  ...
  const hv = useMemo(() => {
    ...
    const e = model.bySlot.get(hover);
    if (!e) return null;
    const x = model.fx(e.slot);
    ...
    const q = e.quote;
    const qs = e.quotes?.length ? e.quotes : q ? [q] : [];
    const quoteText = qs.length
      ? qs.map((x) => `${x.side === "buy" ? "bid" : "ask"} ${fmtPrice(x.price)}`).join(" · ")
      : "no quote";
    const tintSide = e.fill ? e.fill.side : q ? q.side : null;
    return {
      x,
      y: model.fy(e.mid),
      tx: flip ? x - 146 : x + 14,
      ...
      trade: side ? `FILL ${side} ${fmtSol(e.fill!.size, 0)}` : quoteText,
      tint: e.fill ? (e.fill.side === "buy" ? "var(--buy-ink)" : "var(--sell-ink)") : tintSide ? (tintSide === "buy" ? "var(--buy-ink)" : "var(--sell-ink)") : "var(--muted)",
  ...
  const op = late ? null : (d?.operation ?? null);
  const word =
    late ? "Missed the cycle"
    : op === "BUY_SPOT" ? "Buying spot"
    : op === "SELL_SPOT" ? "Selling spot"
    : op === "LONG_PERP" ? "Longing perp"
    : op === "SHORT_PERP" ? "Shorting perp"
    : op === "PAUSE" ? "Paused" : "Holding";
  const wordColor =
    op === "BUY_SPOT" || op === "LONG_PERP"
      ? "var(--buy-ink)"
      : op === "SELL_SPOT" || op === "SHORT_PERP" || op === "PAUSE"
        ? "var(--sell-ink)"
        : late
          ? "var(--late-ink)"
          : "var(--ink)";
  const wordRef = useRef<{ act: string; slot: number }>({ act: op ?? "late", slot: shown?.slot ?? 0 });
  if (wordRef.current.act !== (op ?? "late")) wordRef.current = { act: op ?? "late", slot: shown?.slot ?? 0 };
  const conf = d && !late ? d.confidence : 0;
  const pos = shown?.position;
  const stance =
    !pos || pos.side === "flat"
      ? "flat"
      : `${pos.side} ${fmtSol(pos.size, Number.isInteger(pos.size) ? 0 : 3)}`;
  const pnlSol = shown?.totals?.pnlSol ?? 0;
    ...
    <div className={styles.wrap}>
      <div
        ...
        onPointerMove={(ev) => {
          ...
          setHover(model.bySlot.has(b) ? b : null);
        }}
        onPointerLeave={() => setHover(null)}
      >
        {!model || !shown ? (
          <div className={styles.empty}>waiting for slots…</div>
        ) : (
          <>
                ...
                {hv ? (
                  <g>
                    ...
                    <g transform={`translate(${hv.tx.toFixed(1)},${hv.ty.toFixed(1)})`}>
                      ...
                      <text className={styles.tipBlock} x="12" y="21">{hv.slot}</text>
            ...
            <div className={styles.tl}>
              ...
              <div className={styles.sub}>
                ...
                <span>Phoenix</span>
                <span>{stance}</span>
                <span style={{ color: pnlSol >= 0 ? "var(--pnl-pos)" : "var(--pnl-neg)" }}>
                  p&amp;l {fmtSignedSol(pnlSol, 3)} ({fmtSigned(pnlPct, 2)}%)
                </span>
              </div>
            </div>
            ...
            <div className={styles.tr}>
              <div
                ...
                key={`${wordRef.current.slot}-${op ?? "late"}`}
